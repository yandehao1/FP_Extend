...
namespace RURO.OracleDAL
{
    ...
    public class SysManage1 : ISysManage//这里必须实现接口，否则工厂类创建接口报错
	{
        ...
        public int AddTreeNode(RURO.Model.SysNode model)
		{
        ...
        public void UpdateNode(RURO.Model.SysNode model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// <returns></returns>
        public RURO.Model.SysNode GetNode(int NodeID)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select * from S_Tree ");	
			strSql.Append(" where NodeID=@NodeID");
            ...
            RURO.Model.SysNode node = new RURO.Model.SysNode();
